        ...
        <x-components.data-table class="lg:w-5/6">
            ...
            <x-slot name="body">
                @foreach($this->departments as $department)
                    <tr data-itemId="{{{ $department->getUri() }}}" class="hover:bg-gray-50"  wire:key="{{ 'tr'.$loop->index }}">

                        <x-components.data-table-td>
                            <img src="{{ $department->getLogoUrl() }}">
                        </x-components.data-table-td>

                        <x-components.data-table-td>
                            <a href="{{ $department->getUri() }}" target="_blank" class="text-purple-500 hover:text-indigo-900">{{ $department->getName() }}</a>
                        </x-components.data-table-td>
                        ...
                        @php
                            $employee=\AliSyria\LDOG\OrganizationManager\Employee::retrieveByOrganization($department->getUri());
                        @endphp
                        <x-components.data-table-td>
                            @if($employee)
