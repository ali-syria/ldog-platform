<div>
    <div class="mb-24">
        <x-components.page-header>
            <x-components.page-title>Branches</x-components.page-title>
        </x-components.page-header>
        ...
        <x-components.data-table class="lg:w-5/6">
            ...
            <x-slot name="body">
                @foreach($this->branches as $branch)
                    <tr data-itemId="{{{ $branch->getUri() }}}" class="hover:bg-gray-50"  wire:key="{{ 'tr'.$loop->index }}">

                        <x-components.data-table-td>
                            <img src="{{ $branch->getLogoUrl() }}">
                        </x-components.data-table-td>

                        <x-components.data-table-td>
                            <a href="{{ $branch->getUri() }}" target="_blank" class="text-purple-500 hover:text-indigo-900">{{ $branch->getName() }}</a>
                        </x-components.data-table-td>
                        ...
                        @php
                            $employee=\AliSyria\LDOG\OrganizationManager\Employee::retrieveByOrganization($branch->getUri());
                        @endphp
                        <x-components.data-table-td>
                            @if($employee)
